...
class Hopper(brax_env.Env):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.init_state_noise_scale = kwargs.pop("init_state_noise_scale")
    ...
    def _reset(self, rng: jp.ndarray, noise_scale: float) -> brax_env.State:
        """Resets the environment to an initial state."""
        rng, rng1, rng2 = jp.random_split(rng, 3)
        qpos = self.sys.default_angle() + noise_scale * jp.random_uniform(
            rng1, (self.sys.num_joint_dof,), -0.005, 0.005
        )
        qvel = noise_scale * jp.random_uniform(rng2, (self.sys.num_joint_dof,), -0.005, 0.005)
    ...
    def reset(self, rng: jp.ndarray) -> brax_env.State:
        return self._reset(rng, self.init_state_noise_scale)

    # Return the initial state with no randomness
    def get_initial_state(self) -> brax_env.State:
        # unused rng
        rng = jax.random.PRNGKey(0)
        return self._reset(rng, 0.0)
    ...
    def step(self, state: brax_env.State, action: jp.ndarray) -> brax_env.State:
        ...
        state.metrics.update(
            reward_forward=forward_reward, reward_ctrl=-ctrl_cost, reward_healthy=healthy_reward
        )
