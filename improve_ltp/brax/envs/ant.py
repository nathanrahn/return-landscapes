...
class Ant(env.Env):
    ...
    def _reset(self, rng: jp.ndarray, noise_scale: float) -> env.State:
        """Resets the environment to an initial state."""
        rng, rng1, rng2 = jp.random_split(rng, 3)
        qpos = self.sys.default_angle() + noise_scale * jp.random_uniform(
            rng1, (self.sys.num_joint_dof,), -0.1, 0.1
        )
        qvel = noise_scale * jp.random_uniform(rng2, (self.sys.num_joint_dof,), -0.1, 0.1)
    ...
    def get_initial_state(self) -> env.State:
        # unused rng
