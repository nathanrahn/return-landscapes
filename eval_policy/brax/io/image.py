...
def make_array(stuff):
  qp, camera, sys, width, height, light, ssaa = stuff
  return Image.fromarray(render_array(sys, qp, width, height, light, camera, ssaa))

def expand(x, length):
  return [x] * length

def render_parallel(sys: brax.System,
           qps: List[brax.QP],
           width: int,
           height: int,
           light: Optional[Light] = None,
  ...
  syss, widths, heights, lights, ssaas = [expand(x, len(qps)) for x in (sys, width, height, light, ssaa)]
  frames = pool.map(make_array, list(zip(qps, cameras, syss, widths, heights, lights, ssaas)))
