...
def render2(sys: brax.System, qps: List[brax.QP], qps2: List[brax.QP], height: int = 480) -> str:
  """Returns an HTML page that visualizes the system and qps trajectory, for two trajectories."""
  if any((len(qp.pos.shape), len(qp.rot.shape)) != (2, 2) for qp in qps):
    raise RuntimeError('unexpected shape in qp.')
  d = {
      'config': json_format.MessageToDict(sys.config, True),
  ...
  d = {
      'config': json_format.MessageToDict(sys.config, True),
      'pos': [qp.pos for qp in qps2],
      'rot': [qp.rot for qp in qps2],
  }
  system2 = json.dumps(d, cls=JaxEncoder)

  html = _HTML2.replace('<!-- system json goes here -->', system)
  html = html.replace('<!-- system2 json goes here -->', system2)
  html = html.replace('<!-- viewer height goes here -->', f'{height}px')
  return html
...
<html>
  ...
  <body>
    <script type="application/javascript">
    var system = <!-- system json goes here -->;
    var system2 = <!-- system2 json goes here -->;
    </script>
    <div id="brax-viewer"></div>
    <div id="brax-viewer-2"></div>
    <script type="module">
      import {Viewer} from 'https://cdn.jsdelivr.net/gh/google/brax@v0.0.13/js/viewer.js';
      const domElement = document.getElementById('brax-viewer');
      var viewer = new Viewer(domElement, system);
      const domElement2 = document.getElementById('brax-viewer-2');
      var viewer2 = new Viewer(domElement, system2);
    </script>
  </body>
</html>
"""
