...
class DDPGLearner(object):
    def __init__(
        ...
        self.actor, self.target_actor, self.critic, self.target_critic, self.rng = init_models(
    ...
    def replicate_agent(self, index: int):
        """Replicate every model in the agent (but not the rng) copying from the specified index"""
        self.actor = jax.tree_map(lambda x: np.array(jnp.array(x).at[:].set(x[index])), self.actor)
        self.target_actor = jax.tree_map(lambda x: np.array(jnp.array(x).at[:].set(x[index])), self.target_actor)
        self.critic = jax.tree_map(lambda x: np.array(jnp.array(x).at[:].set(x[index])), self.critic)
        self.target_critic = jax.tree_map(lambda x: np.array(jnp.array(x).at[:].set(x[index])), self.target_critic)
