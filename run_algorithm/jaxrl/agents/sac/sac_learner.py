...
@functools.partial(jax.jit, static_argnames=('discount', 'tau', 'target_entropy', 'num_seeds', 'num_updates'))
...
class SACLearner(object):
    def __init__(self,
        ...
        self.actor, self.critic, self.target_critic, self.temp, self.rng = init_models(jnp.arange(seed,
                                                                                                  seed + num_seeds))
        self.step = 1
    ...
    def update(self, batch: Batch, actor_batch_size: int, critic_batch_size: int, num_updates: int = 1) -> InfoDict:
