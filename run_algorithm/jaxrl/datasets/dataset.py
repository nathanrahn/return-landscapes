...
ExtendedBatch = collections.namedtuple(
    'ExtendedBatch',
    ['qp_states', 'observations', 'actions', 'rewards', 'masks', 'next_observations'])
...
def strip_batch(batch: ExtendedBatch) -> Batch:
    return Batch(batch.observations, batch.actions, batch.rewards,
                 batch.masks, batch.next_observations)
...
class Dataset(object):
    def __init__(self, qp_states: np.ndarray, observations: np.ndarray, actions: np.ndarray,
                 rewards: np.ndarray, masks: np.ndarray,
                 dones_float: np.ndarray, next_observations: np.ndarray,
                 size: int):
        self.qp_states = qp_states
    ...
    def sample(self, batch_size: int) -> ExtendedBatch:
        indx = np.random.randint(self.size, size=batch_size)
        return ExtendedBatch(qp_states=self.qp_states[indx],
