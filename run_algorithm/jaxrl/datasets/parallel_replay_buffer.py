...
class ParallelReplayBuffer:
    ...
    def multiple_insert_to_seed(
        ...
        self.observations[seed, self.insert_index:self.insert_index + len(observation)] = observation
        self.actions[seed, self.insert_index:self.insert_index + len(observation)] = action
        self.masks[seed, self.insert_index:self.insert_index + len(observation)] = mask
        
        next_state = next_state.replace(info={})
        def set_item_for_seed(x, y):
            x[self.insert_index:self.insert_index + len(y), seed] = y

        jax.tree_map(set_item_for_seed, self.next_states, next_state)

        self.insert_index = (self.insert_index + len(observation)) % self.capacity
        self.size = min(self.size + 1, self.capacity)
    ...
    def replicate_buffer(self, index):
        self.observations[:] = self.observations[index]
        self.actions[:] = self.actions[index]
        self.masks[:] = self.masks[index]
        self.next_states = jax.tree_map(lambda x: np.array(jnp.array(x).at[:].set(x[:, [index]])), self.next_states)
    ...
    def load(self, data_path: str):
        ...
        if zip_path.exists():
            ...
            for i in range(self.n_parts):
                with np.load(data_path + f"_chunk_{i}.npz") as npz_file:
